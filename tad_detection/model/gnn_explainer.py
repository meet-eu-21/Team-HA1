...
class GNNExplainer(torch.nn.Module):
    r"""
    Implementation derived from PyTorch Geometric implementation of GNNExplainer: https://pytorch-geometric.readthedocs.io/en/latest/modules/nn.html#torch_geometric.nn.models.GNNExplainer
    ...
    def explain_graph(self, x, edge_index, **kwargs):
        ...
        with torch.no_grad():
            ...
            if self.return_type == 'regression':
                ...
                log_logits = self.__to_log_prob__(out[0])
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            if self.return_type == 'regression':
                loss = self.__loss__(-1, out[0], prediction)
            else:
                log_logits = self.__to_log_prob__(out[0])
    ...
    def explain_node(self, node_idx, x, edge_index, **kwargs):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            if self.return_type == 'regression':
                loss = self.__loss__(mapping, out[0], prediction)
            else:
                log_logits = self.__to_log_prob__(out[0])
